...
Public Class AssetsBrowser
    ...
    Dim WithEvents NewLoadingWindow As New SyncWindow() With {.Title = "Loading XMB files", .ShowActivated = True}
    ...
    Private Sub AssetsBrowser_ContentRendered(sender As Object, e As EventArgs) Handles Me.ContentRendered
        If Not String.IsNullOrEmpty(SelectedDirectory) Then
            ...
            NewLoadingWindow = New SyncWindow() With {.Title = "Loading XMB files", .ShowActivated = True}
        ...
        If AssetFilesListView.SelectedItem IsNot Nothing Then
            ...
            Try
                ...
                ElseIf SelectedAsset.Type = AssetType.XML Then
                    Dim NewAdvancedEditor As New AdvancedEditor() With {.SetSyntax = "XML", .FileContent = File.ReadAllText(SelectedAsset.AssetFilePath), .FilePath = SelectedAsset.AssetFilePath, .Title = "Advanced Editor - " + SelectedAsset.AssetFilePath, .ShowActivated = True}
                    NewAdvancedEditor.Show()
                ElseIf SelectedAsset.Type = AssetType.DIC Then
                    Dim NewAdvancedEditor As New AdvancedEditor() With {.SetSyntax = "DIC", .FileContent = File.ReadAllText(SelectedAsset.AssetFilePath), .FilePath = SelectedAsset.AssetFilePath, .Title = "Advanced Editor - " + SelectedAsset.AssetFilePath, .ShowActivated = True}
                    NewAdvancedEditor.Show()
                End If
    ...
    Private Sub OpenImageEditorImage_MouseLeftButtonDown(sender As Object, e As MouseButtonEventArgs) Handles OpenImageEditorImage.MouseLeftButtonDown
        If AssetFilesListView.SelectedItem IsNot Nothing Then
            Dim SelectedAsset As AssetListViewItem = CType(AssetFilesListView.SelectedItem, AssetListViewItem)
            Dim NewSimpleGraphicsEditor As New SimpleGraphicsEditor() With {.Title = "Editing " + SelectedAsset.AssetFilePath, .LoadedImageFilePath = SelectedAsset.AssetFilePath}
